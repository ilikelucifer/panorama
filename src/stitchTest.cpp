...
//============================================================================================================//
stitchTest stitchTest::creatDefault(bool try_use_gpu, float seamScale, int blendLayer, int test_mode)
{
	stitchTest stitcher;

	stitcher.setSeamScale(seamScale);
// #ifdef HAVE_CUDA //opencv3.2
#if defined(HAVE_OPENCV_GPU) && !defined(DYNAMIC_CUDA_SUPPORT) //opencv2.4.9
	...
	stitcher.setTestMode(test_mode);

	return stitcher;
}


stitchTest::Status stitchTest::stitcher(InputArray src, OutputArray pano, const string& xml_path)
{

	src.getMatVector(imgs_);
	...
	stitchTest::Status status = myComposePanorama(pano);

	if (status != stitchTest::OK)
	{
...
stitchTest::Status stitchTest::myComposePanorama(OutputArray pano)
{
	//相机参数是否合理，需要做检查
#if 0
	...
	double real_work_scale = 1.0;
	double real_seam_scale = 0.4;

	if (testMode_ == 0)//app
	{
		real_work_scale = work_scale_;
	}
	else//cloud
	{
		real_work_scale = work_scale_*0.5;
	}


	//尺度参数放到一起好处理
	double seam_work_aspect = seam_scale_ / real_work_scale;//
	double compose_scale = 1;
	double compose_work_aspect = compose_scale / real_work_scale;//
...
void stitchTest::readCameraParams(const std::string& camera_path)
{
	cv::FileStorage fs;
